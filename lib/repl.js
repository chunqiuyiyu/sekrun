...
export async function runRepl(agent) {
  console.error(`zek - coding agent (DeepSeek V4 Flash)
...
`);
  ...
  try {
    while (true) {
      ...
      if (trimmed === '/ask') {
        askMode = true;
        console.error('(read-only ask mode - /edit to switch back)');
        continue;
      }
      if (trimmed === '/edit' || trimmed === '/agent') {
        askMode = false;
      ...
      if (trimmed.startsWith('/ask ')) {
        ...
        await agent.handleUserMessage(question, true);
        continue;
      }

      await agent.handleUserMessage(trimmed, askMode);
    }
  } catch (error) {
...
function printHelp() {
  ...
  /edit     Return to agent mode (can modify files)
