...
/**
 * Print model reasoning/thinking content in dimmed style.
 */
export function printThinking(text) {
  if (!text) return;
  const dim = '\x1b[2m';
  const reset = '\x1b[0m';
  console.error(dim + '\u2506 Thinking' + reset);
  const lines = String(text).split("\n");
  for (const line of lines) {
    console.error(dim + line + reset);
  }
  console.error(dim + '\u2506' + ' \u2500'.repeat(16) + reset);
}
