 ...
 *   - Press Enter with unclosed brackets/braces/parentheses/quotes
...
export async function readLine(prompt, opts = {}) {
  ...
  let _prevRenderLines = 1;        // lines occupied by last render
  ...
  return new Promise((resolve) => {
    ...
    function render() {
      ...
      // Determine cursor column relative to current line (handles multi-line)
      const beforeCursor = input.slice(0, cursor);
      const lastNewline = beforeCursor.lastIndexOf('\n');
      const currentLineText = lastNewline === -1 ? beforeCursor : beforeCursor.slice(lastNewline + 1);
      const col = (lastNewline === -1 ? promptWidth : 0) + visibleWidth(currentLineText);
      const cursorLine = (beforeCursor.match(/\n/g) || []).length; // 0-indexed line

      // Clear all lines from previous render (including the starting line)
      stdout.write('\r' + eraseInLine(2));
      for (let i = 1; i < _prevRenderLines; i++) {
        stdout.write(csi + 'A' + eraseInLine(2));
      }
      stdout.write(rendered);

      // Update line count for next render
      _prevRenderLines = rendered.split('\n').length;

      // Position cursor — move to correct line then column
      const linesFromBottom = _prevRenderLines - 1 - cursorLine;
      if (linesFromBottom > 0) {
        stdout.write(csi + linesFromBottom + 'A');
      }
      stdout.write(`\r${csi}${col + 1}G`);
    }
     ...
     */
    function commitLine(forceSubmit = false) {
      if (!forceSubmit && hasUnclosedSyntax(input)) {
        // Unclosed brackets/braces/parens/quotes — insert newline instead of submitting
        input += '\n';
        cursor = input.length;
        render();
        return;
    ...
    function onKeypress(str, key) {
      ...
      // Ctrl+Enter / Ctrl+J — force-submit even if syntax is unclosed
      if ((key.ctrl && key.name === 'enter') || (key.ctrl && key.name === 'j')) {
        commitLine(true);
        return;
      }

      // Enter — commit line (or insert newline if unclosed syntax)
      if (key.name === 'return' || key.name === 'enter') {
        const now = Date.now();
        const isRapidFire = (now - _lastEventTime < 50 && _lastEventTime > 0);
        _lastEventTime = now;
        if (isRapidFire) {
          // Rapid Enter — likely from a paste (handles both \r and \n).
          // Skip if input already ends with \n to avoid \r\n → \n\n duplication.
          if (!input.endsWith('\n')) {
            input += '\n';
            cursor = input.length;
          }
