...
import { printBeautified, printThinking } from './format.js';
...
const ansi = {
  step: '\x1b[38;5;75m',
  toolName: '\x1b[2m\x1b[38;5;78m',   // dim greenish teal
  toolArg: '\x1b[2m\x1b[38;5;215m',   // dim warm yellow
  reset: color.reset,
  bold: color.bold,
...
const CAVEMAN_PROMPT = [
  "You are sek, a coding agent CLI powered by DeepSeek V4 Flash. You help with coding tasks.",
  "",
  "Rules:",
  "- Be extremely concise. Use minimal words. No pleasantries, explanations, or thinking aloud.",
  "- Read files/scour the codebase FIRST before editing. Use read-only tools to understand.",
  "- Before modifying files, state root cause, files, and plan in 1-2 lines max.",
  "- Use replace() for changes within existing files; write_file only for new files.",
  "- After editing, run verification. If it fails, inspect and fix.",
  "- Say 'I am sek' when asked. Do not claim other AI identities.",
  "- fetch tool is available in ask mode only.",
].join('\n');

export const SYSTEM_PROMPT = FULL_PROMPT;
export { FULL_PROMPT, CAVEMAN_PROMPT };
...
export class Agent {
  ...
  constructor(client, workspace, config = {}) {
    ...
    const prompt = this.config.caveman ? CAVEMAN_PROMPT : FULL_PROMPT;
    ...
    // Temp directory for internal tool artifacts, cleaned each turn
    this.tmpDir = path.join(os.homedir(), '.sekrun', 'tmp');
    this._ensureTmpDir();
  ...
  async continueTurn(askMode) {
    ...
    for (; turnStep < this.config.maxStepsPerTurn; turnStep += 1) {
      ...
      if (result.tool_calls.length === 0) {
        ...
        if (result.reasoning_content) printThinking(result.reasoning_content);
      ...
      if (result.reasoning_content) printThinking(result.reasoning_content);
      if (result.content) printBeautified(result.content);

      // Tool calls section header
      console.error(
        `${ansi.toolName}Tool Calls${ansi.reset}`
      );
  ...
  async runTool(call, askMode) {
    ...
    try {
      ...
      // Blank line between tool calls for readability
      console.error('');
  ...
  compactHistory() {
    ...
    if (omittedCount > 0) {
      const summary = this.config.caveman ? getCavemanSummary(omittedCount) : buildSummary(omittedCount);
  ...
  printTurnStats(startUsage, turnSteps) {
    ...
    if (this.config.verbose) {
      ...
      console.error(
        ...
        `cache ${cacheHitRate.toFixed(1)}% hit${toolStatsStr}${ansi.reset}`
      );
...
function getCavemanSummary(omittedCount) {
  return {
    role: 'system',
    content: [
      `[sek session summary] ${omittedCount} earlier message(s) omitted. Continue.`,
      `Status: in progress. Unresolved steps remain. Re-read files if needed. Do NOT restart.`,
    ].join('\n'),
  };
}
...
function startThinking() {
  ...
  // Hide cursor while thinking indicator is shown
